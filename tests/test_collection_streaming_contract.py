import json
from pathlib import Path

from ruamel.yaml import YAML

ROOT = Path(__file__).resolve().parents[1]
QS_COLLECTIONS_PATH = ROOT / "static" / "json" / "quickstart_collections.json"
STREAMING_DEFAULT_PATH = ROOT / "config" / "kometa" / "defaults" / "both" / "streaming.yml"


def _load_streaming_collection():
    data = json.loads(QS_COLLECTIONS_PATH.read_text(encoding="utf-8"))
    for group in data:
        for collection in group.get("collections", []):
            if collection.get("id") == "collection_streaming":
                return collection
    raise AssertionError("Missing collection_streaming")


def _field_map(collection):
    return {item["key"]: item for item in collection.get("template_variables", []) if isinstance(item, dict) and item.get("key")}


def _streaming_provider_keys():
    parser = YAML(typ="safe", pure=True)
    parsed = parser.load(STREAMING_DEFAULT_PATH.read_text(encoding="utf-8"))
    data = parsed["dynamic_collections"]["Streaming"]["data"]
    return list(data.keys())


def test_streaming_provider_toggles_match_repo_yaml_data():
    collection = _load_streaming_collection()
    fields = _field_map(collection)

    qs_providers = sorted(key.removeprefix("use_") for key in fields if key.startswith("use_") and key not in {"use_all", "use_separator"})

    assert qs_providers == sorted(_streaming_provider_keys())


def test_streaming_exposes_yaml_verified_shared_fields_and_override_maps():
    collection = _load_streaming_collection()
    fields = _field_map(collection)

    expected_shared = {
        "schedule": {"type": "schedule", "section": "scope_schedule"},
        "name_mapping": {"type": "text_input", "section": "naming"},
        "delete_collections_named": {"type": "string_list", "section": "scope_schedule"},
        "url_poster": {"type": "text_input", "validation_preset": "url", "section": "artwork"},
        "url_background": {"type": "text_input", "validation_preset": "url", "section": "artwork"},
        "url_logo": {"type": "text_input", "validation_preset": "url", "section": "artwork"},
        "url_square_art": {"type": "text_input", "validation_preset": "url", "section": "artwork"},
    }

    for key, metadata in expected_shared.items():
        assert key in fields
        for meta_key, meta_value in metadata.items():
            assert fields[key].get(meta_key) == meta_value

    exclude_field = fields["exclude"]
    assert exclude_field["type"] == "string_list"
    assert exclude_field["validation_preset"] == "streaming_key"
    assert exclude_field["input_mode"] == "select"

    section_defs = collection.get("template_variable_sections") or []
    section_ids = [section.get("id") for section in section_defs if isinstance(section, dict)]
    assert section_ids[:8] == [
        "basics",
        "naming",
        "scope_schedule",
        "child_defaults",
        "child_override_maps",
        "artwork",
        "automation_defaults",
        "automation_override_maps",
    ]
    assert "provider_movistar" in section_ids
    assert "provider_atresplayer" in section_ids
    assert "provider_amc" in section_ids
    assert "provider_filmin" in section_ids
    assert any(section.get("default_open") is True for section in section_defs if section.get("id") == "basics")

    expected_mappings = {
        "child_schedule_overrides": ("schedule_", "string", "child_override_maps", None, None),
        "child_name_mapping_overrides": ("name_mapping_", "string", "child_override_maps", None, None),
        "child_sort_by_overrides": ("sort_by_", "select", "child_override_maps", None, None),
        "child_delete_collections_named_overrides": ("delete_collections_named_", "string_list", "child_override_maps", None, None),
        "child_discover_with_overrides": ("discover_with_", "string", "child_override_maps", None, None),
        "child_url_poster_overrides": ("url_poster_", "string", "artwork", "url", None),
        "child_url_background_overrides": ("url_background_", "string", "artwork", "url", None),
        "child_url_logo_overrides": ("url_logo_", "string", "artwork", "url", None),
        "child_url_square_art_overrides": ("url_square_art_", "string", "artwork", "url", None),
        "child_radarr_folder_overrides": ("radarr_folder_", "string", "automation_override_maps", None, ["movie"]),
        "child_radarr_tag_overrides": ("radarr_tag_", "string_list", "automation_override_maps", None, ["movie"]),
        "child_item_radarr_tag_overrides": ("item_radarr_tag_", "string_list", "automation_override_maps", None, ["movie"]),
        "child_radarr_monitor_overrides": ("radarr_monitor_", "boolean", "automation_override_maps", None, ["movie"]),
        "child_radarr_upgrade_existing_overrides": ("radarr_upgrade_existing_", "boolean", "automation_override_maps", None, ["movie"]),
        "child_radarr_monitor_existing_overrides": ("radarr_monitor_existing_", "boolean", "automation_override_maps", None, ["movie"]),
        "child_radarr_search_overrides": ("radarr_search_", "boolean", "automation_override_maps", None, ["movie"]),
        "child_sonarr_folder_overrides": ("sonarr_folder_", "string", "automation_override_maps", None, ["show"]),
        "child_sonarr_tag_overrides": ("sonarr_tag_", "string_list", "automation_override_maps", None, ["show"]),
        "child_item_sonarr_tag_overrides": ("item_sonarr_tag_", "string_list", "automation_override_maps", None, ["show"]),
        "child_sonarr_monitor_overrides": ("sonarr_monitor_", "select", "automation_override_maps", None, ["show"]),
        "child_sonarr_upgrade_existing_overrides": ("sonarr_upgrade_existing_", "boolean", "automation_override_maps", None, ["show"]),
        "child_sonarr_monitor_existing_overrides": ("sonarr_monitor_existing_", "boolean", "automation_override_maps", None, ["show"]),
        "child_sonarr_search_overrides": ("sonarr_search_", "boolean", "automation_override_maps", None, ["show"]),
    }

    for key, (child_prefix, value_kind, section, value_validation_preset, media_types) in expected_mappings.items():
        assert key in fields
        field = fields[key]
        assert field["type"] == "mapping_list"
        assert field["dynamic_child_prefix"] == child_prefix
        assert field["dynamic_child_value_kind"] == value_kind
        assert field["key_validation_preset"] == "streaming_key"
        assert field["key_input_mode"] == "select"
        assert field["section"] == section
        if value_validation_preset is None:
            assert "validation_preset" not in field
        else:
            assert field["validation_preset"] == value_validation_preset
        if media_types is None:
            assert "media_types" not in field
        else:
            assert field["media_types"] == media_types


def test_streaming_missing_provider_fields_are_grouped_in_provider_sections():
    fields = _field_map(_load_streaming_collection())

    for provider in ("movistar", "atresplayer", "amc", "filmin"):
        section_id = f"provider_{provider}"
        assert fields[f"use_{provider}"]["section"] == section_id
        assert fields[f"name_{provider}"]["section"] == section_id
        assert fields[f"summary_{provider}"]["section"] == section_id
        assert fields[f"sync_mode_{provider}"]["section"] == section_id
        assert fields[f"sync_mode_{provider}"]["default"] == ""
        assert fields[f"radarr_add_missing_{provider}"]["section"] == section_id
        assert fields[f"sonarr_add_missing_{provider}"]["section"] == section_id
        assert fields[f"visible_home_{provider}"]["section"] == section_id
        assert fields[f"visible_library_{provider}"]["section"] == section_id
        assert fields[f"visible_shared_{provider}"]["section"] == section_id
        assert fields[f"hub_priority_{provider}"]["section"] == section_id
        assert fields[f"hub_priority_{provider}"]["requires_any_of_template_keys"] == [
            f"visible_home_{provider}",
            f"visible_library_{provider}",
            f"visible_shared_{provider}",
        ]
